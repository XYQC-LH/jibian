...
@Injectable()
export class T8GptImage2GenerationsAdapter implements SourceAdapter {
  readonly sourceId = "t8-gpt-image-2-generations";
  private readonly upstreamModelName = "gpt-image-2";
  private readonly defaultResolution = "1K";
  ...
  async generate(input: StandardGenerateInput): Promise<StandardGenerateOutput> {
    ...
    if (!endpoint || !apiKey) {
      return { ok: false, errorMessage: "T8 GPT Image 2 Generations missing T8_BASE_URL or T8_API_KEY" };
    }
    ...
    if (!response.ok) {
      return { ok: false, errorMessage: this.errorMessage(body, `T8 generations request failed: ${response.status}`) };
    }

    const imageUrl = this.extractImageUrl(body);
    if (!imageUrl) {
      return { ok: false, errorMessage: "T8 generations response missing image URL" };
    }
