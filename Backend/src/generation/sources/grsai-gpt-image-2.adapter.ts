  ...
  async generate(input: StandardGenerateInput): Promise<StandardGenerateOutput> {
    ...
    for (let index = 0; index < 120; index += 1) {
      ...
      if (status === "failed" || status === "violation") {
        return { ok: false, errorMessage: this.errorMessage(pollBody, "GRSAI generation failed"), upstreamJobId };
      }
    ...
    return { ok: false, errorMessage: "GRSAI polling timeout", upstreamJobId };
  }
