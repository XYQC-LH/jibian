...
小程序不再本地决定积分扣减、生成成功、兑换码有效、手机号绑定成功和账号注销成功。这些状态由后端返回，小程序只展示结果。
...
管理员端不在浏览器里执行核心业务规则。所有上架、停用、自动审核开关、重跑、退款和模型源配置动作都调用后端 action API，由后端校验权限并写入业务状态。
...
- `Sidebar`：模块包含名称、图标、描述、权限 key 和高亮色；即变第一版模块映射为 Dashboard、Templates、Tasks、Moderation、ModelProviders、Credits、Users。
...
- `Templates`：模板列表、创建模板、编辑封面、价格、结果数量和单一基础 prompt。
- `Tasks`：生成任务列表，支持按用户、模板、状态、时间和失败原因筛选。
- `TaskDetail`：展示 task、source runs、review records 和积分流水。
- `Moderation`：自动审核概览、事件列表和审核开关；不提供人工审核池、人工通过或人工驳回。
...
-> 查看命中策略、阶段和失败原因
-> 必要时调整自动审核总开关
-> 回到任务详情查看 task、review_record 和退款状态
```
...
- 管理员端所有危险 action 使用二次确认，并由后端返回最新业务状态。
...
5. 管理员端补齐任务详情、source runs、自动审核观测和模型源管理。
...
- 当前端触发上架、审核开关、重跑、停用 provider 和退款类动作时，后端更新业务状态，管理员端刷新后展示最新结果。
